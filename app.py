...
# ==========================================
async def process_video_async(file_bytes, api_key, status_box, progress_bar):
    try:
        ...
        with tempfile.TemporaryDirectory() as work_dir:
            ...
            if not chunk_files:
                return None, "ဗီဒီယိုထဲတွင် အသံဖိုင် (Audio Stream) မပါဝင်ပါ။"
            ...
            if not all_subs:
                return None, "ဘာသာပြန်ဆိုရန် အသံစာသား ရှာမတွေ့ပါ။ API Key သေဆုံးနေခြင်းလည်း ဖြစ်နိုင်ပါသည်။"
            ...
            if os.path.exists(out_video):
                ...
                return None, "ဗီဒီယို ပေါင်းစပ်မှု မအောင်မြင်ပါ။"
...
st.title("🎬 AI Myanmar Video Translator")
...
if uploaded_file is not None:
    ...
    if st.button("🚀 ဗီဒီယိုအား မြန်မာမှုပြုလုပ်မည်", type="primary"):
        ...
        # Async function ကို Streamlit ရဲ့ Main Thread ပေါ်တွင် ခေါ်ယူ Run ခြင်း
        result_video_bytes, error_message = asyncio.run(process_video_async(file_bytes, current_api_key, status_box, progress_bar))
        ...
        if result_video_bytes:
            ...
            status_box.error(f"❌ အမှားအယွင်းဖြစ်ပွားသည် - {error_message}")
